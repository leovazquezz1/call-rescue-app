...
const Widgets = () => {
  return (
    <React.Fragment>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-space">
        {data.map((item) => (
          <div className="relative overflow-hidden card" key={item.id}>
            ...
            <div className="relative card-body">
              <h6 className="uppercase text-14">{item.plan}</h6>
              <div className="flex items-center justify-center h-32">
                <div>
                  {item.trending ? (
                    <TrendingUp className="mx-auto mb-2 text-green-500 size-7"></TrendingUp>
                  ) : (
                    <TrendingDown className="mx-auto mb-2 text-red-500 size-7"></TrendingDown>
                  )}
                  <h3>{item.percent}</h3>
                </div>
              </div>
              <h6>
                <UserRound className="inline-block ltr:mr-1 rtl:ml-1 size-4 mb-0.5"></UserRound>
                {item.price}
              </h6>
            </div>
          </div>
